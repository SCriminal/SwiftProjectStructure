...
extension RequestApi {
    ...
    class func requestLogin(account:String, password:String, delegate:RequestDelegate?, success:(( Dictionary<AnyHashable,Any?>?,AnyObject?)->Void)?, failure:((String?,AnyObject?)->Void)?) {
    ...
    ///admin/module/list/total?scope=7&page=1&count=50
    class func requestModuleList(delegate:RequestDelegate?, success:(( Dictionary<AnyHashable,Any?>?,AnyObject?)->Void)?, failure:((String?,AnyObject?)->Void)?) {
        let dic: [String:Any?] = ["scope":7,
                                  "page":1,
                                  "count":500]
        self.getUrl(URL: "/admin/module/list/total", delegate: delegate, parameters: dic, success: success, failure: failure)
    }
    
    class func requestUpdateModel(model: ModelModule, delegate:RequestDelegate?, success:(( Dictionary<AnyHashable,Any?>?,AnyObject?)->Void)?, failure:((String?,AnyObject?)->Void)?) {
        let dic: [String:Any?] = ["goMode":model.goMode,
                                  "isOpen":model.isOpen,
